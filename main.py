...
def get_largest_expenses(
    ...
    """Get the largest expenses in a date range."""
...
def monthly_summary(year, month):
    ...
    start_date = f"{int(year):04d}-{month:02d}-01"

    if month == 12:
        end_date = f"{int(year) + 1:04d}-01-01"
    else:
        end_date = f"{int(year):04d}-{month + 1:02d}-01"
...
def get_budget_status(month):
    ...
    for budget in budgets:
        ...
        spent = cursor.fetchone()["COALESCE(SUM(amount), 0)"]
...
def financial_insights(
    ...
    if category_data:
        ...
        top_category = {
            ...
            "amount": float(category_data[0]["amount"])
        }
